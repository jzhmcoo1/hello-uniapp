...
function calYAxisData (series, opts, config) {
  ...
  const ranges = getYAxisTextList(series, opts, config, columnstyle.type)
  let yAxisWidth = config.yAxisWidth
  const yAxisFontSize = opts.yAxis.fontSize || config.fontSize
  const rangesFormat = ranges.map(function (item) {
...
function drawRingTitle (opts, config, context, center) {
  ...
  if (subtitle) {
    ...
    const startX = center.x - textWidth / 2 + (opts.subtitle.offsetX || 0)
    let startY = center.y + subtitlefontSize / 2 + (opts.subtitle.offsetY || 0)
    if (title) {
  ...
  if (title) {
    ...
    let _startY = center.y + titlefontSize / 2 + (opts.title.offsetY || 0)
    if (subtitle) {
...
function drawPointText (points, series, config, context) {
  ...
  points.forEach(function (item, index) {
    if (item !== null) {
      ...
      const formatVal = series.format ? series.format(value) : value
...
function drawGaugeLabel (gaugeOption, radius, centerPosition, opts, config, context) {
  ...
  for (let i = 0; i < gaugeOption.splitLine.splitNumber + 1; i++) {
    ...
    const labelText = gaugeOption.labelFormat ? gaugeOption.labelFormat(nowNumber) : nowNumber
...
function drawPieText (series, opts, config, context, radius, center) {
  ...
  const seriesConvert = series.map(function (item) {
    const text = item.format ? item.format(+item._proportion_.toFixed(2)) : util.toFixed(item._proportion_.toFixed(4) * 100) + '%'
...
function drawToolTip (textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
  ...
  const legendWidth = 4 * opts.pixelRatio
  const legendMarginRight = 5 * opts.pixelRatio
  const arrowWidth = 8 * opts.pixelRatio
  ...
  textList.forEach(function (item, index) {
    if (item.color !== null) {
      ...
      const startY = offset.y + (config.toolTipLineHeight - config.fontSize) / 2 + config.toolTipLineHeight * index +
...
function drawYAxisTitle (title, opts, config, context) {
  const startX = config.xAxisHeight + (opts.height - config.xAxisHeight - measureText(title)) / 2
  context.save()
  context.beginPath()
  context.setFontSize(config.fontSize)
...
function drawAreaDataPoints (series, opts, config, context) {
  ...
  series.forEach(function (eachSeries, seriesIndex) {
    ...
    if (opts.dataPointShape !== false) {
      const shape = config.dataPointShape[seriesIndex % config.dataPointShape.length]
...
function drawLineDataPoints (series, opts, config, context) {
  const process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1
  const lineOption = opts.extra.line || {
    type: 'straight',
    width: 2
  }
...
function drawXAxis (categories, opts, config, context) {
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    const xAxisFontSize = opts.xAxis.fontSize || config.fontSize
    if (config._xAxisTextAngle_ === 0) {
      newCategories.forEach(function (item, index) {
        let offset = -measureText(item, xAxisFontSize) / 2
        if (boundaryGap == 'center') {
...
function drawPieDataPoints (series, opts, config, context) {
  ...
  const activeRadius = pieOption.activeRadius
...
function drawRoseDataPoints (series, opts, config, context) {
  ...
  const activeRadius = roseOption.activeRadius
...
function drawRadarDataPoints (series, opts, config, context) {
  ...
  let radius = Math.min(centerPosition.x - (getMaxTextListLength(opts.categories) + config.radarLabelTextMargin),
  ...
  const _loop = function _loop (i) {
    ...
    coordinateAngle.forEach(function (angle, index) {
      const pos = convertCoordinateOrigin(radius / config.radarGridCount * i * Math.cos(angle), radius / config.radarGridCount *
...
function drawMapDataPoints (series, opts, config, context) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (opts.dataLabel == true) {
      const centerPoint = data[i].properties.centroid
      if (centerPoint) {
...
function drawCharts (type, opts, config, context) {
  ...
  const _calLegendData = calLegendData(seriesMA, opts, config, opts.chartData)
  ...
  case 'line':
    this.animationInstance = new Animation({
      ...
      onProcess: function onProcess (process) {
        ...
        const minRange = _drawLineDataPoints.minRange
        const maxRange = _drawLineDataPoints.maxRange
  ...
  case 'mix':
    this.animationInstance = new Animation({
      ...
      onProcess: function onProcess (process) {
        ...
        const minRange = _drawMixDataPoints.minRange
        const maxRange = _drawMixDataPoints.maxRange
        opts.chartData.xAxisPoints = xAxisPoints
        opts.chartData.calPoints = calPoints
        opts.chartData.eachSpacing = eachSpacing
        drawYAxis(series, opts, config, context)
  ...
  case 'column':
    this.animationInstance = new Animation({
      ...
      onProcess: function onProcess (process) {
        ...
        const minRange = _drawColumnDataPoints.minRange
        const maxRange = _drawColumnDataPoints.maxRange
        opts.chartData.xAxisPoints = xAxisPoints
        opts.chartData.calPoints = calPoints
        opts.chartData.eachSpacing = eachSpacing
        drawYAxis(series, opts, config, context)
  ...
  case 'area':
    this.animationInstance = new Animation({
      ...
      onProcess: function onProcess (process) {
        ...
        const minRange = _drawAreaDataPoints.minRange
        const maxRange = _drawAreaDataPoints.maxRange
        opts.chartData.xAxisPoints = xAxisPoints
        opts.chartData.calPoints = calPoints
        opts.chartData.eachSpacing = eachSpacing
        drawYAxis(series, opts, config, context)
  ...
  case 'candle':
    this.animationInstance = new Animation({
      ...
      onProcess: function onProcess (process) {
        ...
        const minRange = _drawCandleDataPoints.minRange
        const maxRange = _drawCandleDataPoints.maxRange
        opts.chartData.xAxisPoints = xAxisPoints
        opts.chartData.calPoints = calPoints
        opts.chartData.eachSpacing = eachSpacing
        drawYAxis(series, opts, config, context)
...
Charts.prototype.showToolTip = function (e) {
  ...
  if (this.opts.type === 'mix') {
    ...
    if (index > -1) {
      ...
      var seriesData = getSeriesDataItem(this.opts.series, index)
      if (seriesData.length !== 0) {
        const _getMixToolTipData = getMixToolTipData(seriesData, this.opts.chartData.calPoints, index, this.opts.categories, option)
...
Charts.prototype.scroll = function (e) {
  ...
  if (touches && this.opts.enableScroll === true) {
    ...
    const validDistance = calValidDistance(currentOffset + _distance, this.opts.chartData, this.config, this.opts)
