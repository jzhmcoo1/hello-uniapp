describe('pages/component/view/view.vue', () => {
  ...
  /**
	 ...
	 */
  it('view 组件标题', async () => {
    const view = await page.$('.common-page-head-title')
  ...
  /**
	 ...
	 */
  it('view 个数', async () => {
    const viewLen = await page.$$('.uni-flex')
    expect(viewLen.length).toBe(18)
    const viewItemLen = await page.$$('.flex-item')
  ...
  it('view 前三个元素颜色', async () => {
    const viewRed = await page.$('.uni-bg-red')
